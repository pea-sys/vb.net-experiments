...
#Region "select-case-tuple"
        Dim words = {"aPPLE", "BlUeBeRrY", "cHeRry"}
        Dim upperLowerWords = From w In words Select New With {Key .Upper = w.ToUpper(), Key .Lower = w.ToLower()}
...
#Region "select-new-type-tuple"
        Dim numbers = {5, 4, 1, 3, 9, 8, 6, 7, 2, 0}
        Dim strings = {"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"}
        Dim digitOddEvens = From n In numbers Select New With {Key .Digit = strings(n), Key .Even = n Mod 2 = 0}
...
#Region "select-subset-properties"
        Dim products As List(Of Product) = GetProductList()
        Dim productInfos = From p In products Select New With {Key .ProductName = p.ProductName, Key .Category = p.Category, Key .Price = p.UnitPrice}
...
#Region "select-with-index"
        Dim numbers = {5, 4, 1, 3, 9, 8, 6, 7, 2, 0}
        Dim numsInPlace = numbers.Select(Function(num, index) New With {Key .num = num, Key .InPlace = num = index})
